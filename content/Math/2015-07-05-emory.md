...
![Picture of an elliptic curve.]({static}/images/elliptic-curve.png)
...
![Group law on an elliptic curve.]({static}/images/elliptic-add.png)

Letting $A = (A, +)$ be the associated abelian group, we can look at the _endomorphisms_ of $E$ (that is,
homomorphisms $A \rightarrow A$).
These form a [ring](https://en.wikipedia.org/wiki/Endomorphism_ring), which we denote $\text{End }(E)$.
An example of such an endomorphism is $a \mapsto n \cdot a$ for an integer $n$ (meaning $a+\dots+a$,
$n$ times). In this way, we see that $\mathbb Z \subseteq \text{End }(E)$.

Most of the time we in fact have $\text{End }(E) \cong \mathbb Z$.
But on occasion, we will find that $\text{End }(E)$ is congruent to $\mathcal O_K$,
...
we have $\text{End }(E) \cong \mathcal O_K$ where $K$ is some quadratic
